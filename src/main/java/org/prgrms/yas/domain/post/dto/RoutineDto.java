...
public class RoutineDto {
	...
	public RoutineDto(final Routine routine) {
		...
		this.category = routine.getRoutineCategory().stream().map(CategoryDto::categoryToString).collect(Collectors.toList());
		this.missions = routine.getMissions().stream().map(MissionDto::new).collect(Collectors.toList());
	}
}
